{foreach from=$TraderTeamCombatResults.Traders key=AccountID item=TraderResults}
	...
	{else}
		{foreach from=$TraderResults.Weapons item=WeaponResults}
			...
			{$ShootingPlayer->getDisplayName()} fires their {$ShootingWeapon->getName()} at{if $ShotHit && $ActualDamage.TargetAlreadyDead} the debris that was once{/if} the forces{*
		...
		{if isset($TraderResults.Drones)}
			...
			{if $ActualDamage.NumCDs > 0}{assign var=DamageTypes value=$DamageTypes+1}{/if}
			{if $ActualDamage.NumSDs > 0}{assign var=DamageTypes value=$DamageTypes+1}{/if}
			{$ShootingPlayer->getDisplayName()} {if $WeaponDamage.Launched == 0}fails to launch their combat drones{else}launches <span class="yellow">{$WeaponDamage.Launched}</span> combat drones at{if $ActualDamage.TargetAlreadyDead} the debris that was once{/if} the forces{*
				*}{if !$ActualDamage.TargetAlreadyDead} {*
