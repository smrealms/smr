(function() {
		...
		if(highlighted.length==0)
		{
			...
			if(moves != null)
			{
				for(i=0; i < moves.length; i++)
				{
					...
					ele[0].onclick = ele[0].onClick = (function(data){return function(){submitMove(data)}})({x:x,y:y,toX:toX,toY:toY});
	...
	unhighlightMoves = function() {
		...
		while(h = highlighted.pop()) {
			...
			h.ele[0].onclick = (function(x,y){return function(){highlightMoves(x,y)}})(h.x,h.y);
		}
	}
