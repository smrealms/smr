...
while ($db->next_record()) {
	...
	$PHP_OUTPUT.=($temp[$db->f('alliance_id_1')]);
	$PHP_OUTPUT.=('</span>.  Terms as follows:<br /><ul>');
	if ($db->f('trader_assist')) $PHP_OUTPUT.=('<li>Assist - Trader Attacks</li>');
	if ($db->f('trader_defend')) $PHP_OUTPUT.=('<li>Defend - Trader Attacks</li>');
	if ($db->f('trader_nap')) $PHP_OUTPUT.=('<li>Non Aggression - Traders</li>');
	if ($db->f('raid_assist')) $PHP_OUTPUT.=('<li>Assist - Planet & Port Attacks</li>');
	if ($db->f('planet_nap')) $PHP_OUTPUT.=('<li>Non Aggression - Planets</li>');
	if ($db->f('forces_nap')) $PHP_OUTPUT.=('<li>Non Aggression - Forces</li>');
	if ($db->f('aa_access')) $PHP_OUTPUT.=('<li>Alliance Account Access</li>');
	if ($db->f('mb_read')) $PHP_OUTPUT.=('<li>Message Board Read Rights</li>');
	if ($db->f('mb_write')) $PHP_OUTPUT.=('<li>Message Board Write Rights</li>');
	if ($db->f('mod_read')) $PHP_OUTPUT.=('<li>Message of the Day Read Rights</li>');
	if ($db->f('planet_land')) $PHP_OUTPUT.=('<li>Planet Landing Rights</li>');
	$PHP_OUTPUT.=('</ul>');
