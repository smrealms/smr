...
if ($db->nf() > 0) {
    ...
    while ($db->next_record()) {
        ...
        $PHP_OUTPUT.= $db->f('sector') . ' (' . $db->f('galaxy_name');
		$PHP_OUTPUT.= ')</td><td class="shrink center">';
        $PHP_OUTPUT.= $db->f('combat_drones');
        $PHP_OUTPUT.= '</td><td class="shrink center">';
		$PHP_OUTPUT.= $db->f('scout_drones');
		$PHP_OUTPUT.= '</td><td class="shrink center">';
		$PHP_OUTPUT.= $db->f('mines');
		$PHP_OUTPUT.= '</td><td class="shrink nowrap">';
        $PHP_OUTPUT.= date('n/j/Y g:i:s A', $db->f('expire_time'));
        $PHP_OUTPUT.= '</td></tr>';
    }
	$PHP_OUTPUT.= '</table>';
}
