<?
echo 'TELL PAGE YOU SAW THIS MESSAGE (TRADER_ATTACK_PROCCESSING)';
...
//	$db->query('SELECT * FROM macro_check WHERE account_id = '.$player->getAccountID());
//	if($db->next_record())
//		$db->query('UPDATE macro_check SET good = good + 1 WHERE account_id = '.$player->getAccountID());
//	else
//		$db->query('REPLACE INTO macro_check (account_id, good) VALUES ($player->getAccountID(), 1)');
//	
//} elseif (!$var['legit']) {
//	
//	$db->query('SELECT * FROM macro_check WHERE account_id = '.$player->getAccountID());
//	if($db->next_record())
//		$db->query('UPDATE macro_check SET bad = bad + 1 WHERE account_id = '.$player->getAccountID());
//	else
//		$db->query('REPLACE INTO macro_check (account_id, bad) VALUES ($player->getAccountID(), 1)');
//		
//}
//
////this might help
...
//
//$db->query('SELECT * FROM player ' .
//		   'WHERE sector_id = '.$player->getSectorID().' AND ' .
...
//		$db->query('UPDATE kills SET processed = 'TRUE' WHERE game_id = '.$player->getGameID().' AND dead_id = $killed_id AND killer_id = $killer_id');
//
//		// give table free
//		$dead_ppl->unlock();
//
//	if ($debug) $PHP_OUTPUT.=('<p>report any errors on that page to spock</p>');
//
//		// create player object
//		// JUST FOR READING!!!
//		// DO NOT CHANGE SOMETHING IN THERE!
//		$killed =& SmrPlayer::getPlayer($killed_id, SmrSession::$game_id);
//		$killer =& SmrPlayer::getPlayer($killer_id, SmrSession::$game_id);
//
...
//
//	if ($debug) $PHP_OUTPUT.=('insurance<br>');
...
//		}
//
//		$db->query('UPDATE player SET credits = $credits WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('turns<br>');
//		// speed for pod
//		$pod_speed = 7;
//
//		// adapt turns
...
//
//		// update turns
//		$db->query('UPDATE player SET turns = $turns WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('attacker xp<br>');
//		/*
//		$e = experience gained
...
//		$gained_exp = $e;
//		$db->query('UPDATE player SET experience = experience + $gained_exp WHERE account_id = $killer_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('defender xp<br>');
//		$percentage = 20 + (($killed->level_id - $killer->level_id) / 2);
//		if ($percentage > 0) {
//
//			$killed_exp = round($killed->experience * $percentage / 100);
//			$db->query('UPDATE player SET experience = experience - $killed_exp WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//		} else
//			$killed_exp = 0;
//
//	if ($debug) $PHP_OUTPUT.=('hof stats<br>');
//		// update his stats
//		$killer->update_stat('kills', 1);
//		$killer->update_stat('kill_exp', $gained_exp);
//		$killer->update_stat('traders_killed_exp', $killed->experience);
//		
//	if ($debug) $PHP_OUTPUT.=('alliance updating stats<br>');
...
//
//	if ($debug) $PHP_OUTPUT.=('alliance kills<br>');
//		// when he's in an alliance increase their kills
//		if (!empty($killer->alliance_id))
//			$db->query('UPDATE alliance SET alliance_kills = alliance_kills + 1 WHERE game_id = '.$player->getGameID().' AND alliance_id = $killer->alliance_id');
//
//	if ($debug) $PHP_OUTPUT.=('giving attacker money<br>');
//		// give him our money
//		$db->query('UPDATE player SET credits = credits + $killed->credits WHERE account_id = $killer_id AND game_id = '.$player->getGameID());
//		$killer->update_stat('money_gained',$killed->credits);
//
//	if ($debug) $PHP_OUTPUT.=('credit the kill to attacker<br>');
//		// add a kill
//		$db->query('UPDATE player SET kills = kills + 1 WHERE account_id = $killer_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('send message to attacker<br>');
//		// send dead msg
//		$killed->sendMessage($killer->account_id, $PLAYERMSG,
//							  $db->escape_string('You <span style="color:red;">DESTROYED</span> $killed_name in Sector&nbsp<span style="color:blue;">#$curr_sector</span>', false));
//
//	if ($debug) $PHP_OUTPUT.=('send message to defender<br>');
//		//send them a nice message
//		$killer->sendMessage($killed_id, $PLAYERMSG,
//							  $db->escape_string('You were <span style="color:red;">DESTROYED</span> by $killer_name in Sector&nbsp<span style="color:blue;">#$curr_sector</span>', false));
//
//	if ($debug) $PHP_OUTPUT.=('change alignment<br>');
//		// now we change align
...
//		$db->query('UPDATE player SET alignment = alignment + $modifier WHERE account_id = $killer_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('military payment?<br>');
...
//		else
//			$military_pay = 0;
//		$db->query('UPDATE player SET military_payment = military_payment + $military_pay WHERE account_id = $killer_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('bounty?<br>');
//		$db2 = new SMR_DB();
//
//		// can we claim a bounty?
//		$db->query('SELECT * FROM bounty WHERE game_id = '.$player->getGameID().' AND account_id = $killed_id AND claimer_id = 0');
//		while ($db->next_record()) {
//
//			//we have a winner
//			$bounty_id = $db->f('bounty_id');
//
//			$db2->query('UPDATE bounty SET claimer_id = $killer_id WHERE game_id = '.$player->getGameID().' AND account_id = $killed_id AND bounty_id = $bounty_id');
//
//		}
//
...
//							   'VALUES ($killer_id, $player->getGameID(), NULL, '.$db->escapeString($place', $amount, 0, $time)');
//
//				}
//
//			}
...
//		}
//
//	if ($debug) $PHP_OUTPUT.=('get rid of plotted course<br>');
//		// forget plotted course
//		$db->query('DELETE FROM player_plotted_course WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('news entry<br>');
...
//							'.$db->escapeString($killed_name ($killed->race_name)';
...
//							'.$db->escapeString($killer_name ($killer->race_name)';
//							
//			$db2->query('SELECT * FROM ship_has_name WHERE game_id = '.$player->getGameID().' AND ' .
...
//		} else {
//			
//			$news_message = '.$db->escapeString($killed_name';
//			$db2->query('SELECT * FROM ship_has_name WHERE game_id = '.$player->getGameID().' AND ' .
//					'account_id = $killed->account_id');
//			if ($db2->next_record()) {
//				
...
//				   'VALUES($player->getGameID(), ' . time() . ', ' . $db->escape_string($news_message, false) . ')');
//
//	if ($debug) $PHP_OUTPUT.=('increase sector battles<br>');
//		// the sector saw a battle now
//		$db->query('UPDATE sector SET battles = battles + 1 WHERE game_id = '.$player->getGameID().' AND sector_id = $curr_sector');
//
//	if ($debug) $PHP_OUTPUT.=('update alliance deaths<br>');
//		// if we are in an alliance we increase their deaths
//		if ($killed->alliance_id != 0)
//			$db->query('UPDATE alliance SET alliance_deaths = alliance_deaths + 1 WHERE game_id = '.$player->getGameID().' AND alliance_id = $killed->alliance_id');
//
//	if ($debug) $PHP_OUTPUT.=('credit hof death<br>');
//		// record death stat
//		$killed->update_stat('deaths', 1);
//
//	if ($debug) $PHP_OUTPUT.=('reset hof (turns used)<br>');
...
//
//	if ($debug) $PHP_OUTPUT.=('back to hq<br>');
//		// send him to his hq
//		//send out scout msgs
//		$sector->leaving_sector();
//		$db->query('UPDATE player SET sector_id = ' . $killed->get_home() . ' WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('increase player deaths<br>');
//		// register deaths
//		$db->query('UPDATE player SET deaths = deaths + 1 WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//		
//	if ($debug) $PHP_OUTPUT.=('Update Ship Cost HoF<br>');
...
//
//	if ($debug) $PHP_OUTPUT.=('delete all weapons, hardware, illusion and cloak<br>');
//		// get him a pod
//		$db->query('DELETE FROM ship_has_weapon WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//		$db->query('DELETE FROM ship_has_cargo WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//		$db->query('DELETE FROM ship_has_hardware WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//		$db->query('DELETE FROM ship_has_illusion WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//		$db->query('DELETE FROM ship_is_cloaked WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//	if ($debug) $PHP_OUTPUT.=('give shields, armor and cargo<br>');
//		$db->query('INSERT INTO ship_has_hardware (account_id, game_id, hardware_type_id, amount, old_amount) ' .
//				   'VALUES($killed_id, $player->getGameID(), HARDWARE_SHIELDS, 50, 50)');
//		$db->query('INSERT INTO ship_has_hardware (account_id, game_id, hardware_type_id, amount, old_amount) ' .
//				   'VALUES($killed_id, $player->getGameID(), HARDWARE_ARMOR, 50, 50)');
//		$db->query('INSERT INTO ship_has_hardware (account_id, game_id, hardware_type_id, amount, old_amount) ' .
//				   'VALUES($killed_id, $player->getGameID(), HARDWARE_CARGO, 5, 5)');
//
//	if ($debug) $PHP_OUTPUT.=('make it a pod<br>');
//		// actual pod
//		$db->query('UPDATE player SET ship_type_id = 69 WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//		// did we attack and die?
//		// we don't get a pod screen then
//		if ($killed_id == $attacker_id)
//			$db->query('UPDATE player SET dead = 'FALSE' WHERE account_id = $killed_id AND game_id = '.$player->getGameID());
//
//	} else {
//
//		// stop looping
...
//} else {
//
//	// we cannot forward if something was echoed on that page
//
//}
