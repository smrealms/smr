...
function process_attacker($attacker,&$defenders,&$players,&$weapons) {
	...
	for($i=0;$i<$num_weapons;++$i) {
		...
		$players[$defender][ARMOUR] -= $result[2];
...
function process_weapon($weapon,$attacker,$defender,&$players,&$weapons) {
	...
	if($players[$defender][ARMOUR] != 0 ) {
		...
		if($potential_damage) {
			// Have we produced more damage than there are shields remaining?
			if($potential_damage >= $players[$defender][ARMOUR]) {
				$result[2] = $players[$defender][ARMOUR];
				// Final hit
				$result[4] = FINAL_HIT;
			}
			else {
...
function players_init(&$attackers, &$defenders) {
	...
	else if($db->f('sector_id') != $player->getSectorID()) {
		$container=array();
		$container['url'] = 'skeleton.php';
		$container['body'] = 'current_sector.php';
	...
	for($i=0;$i<2;++$i) {
		...
		foreach($fleets[$i] as $attacker) {
			...
			foreach($players[$attacker][RESULTS] as $result) {
				...
				else if ($result[4] == SHIELD_ON_ARMOUR) {
...
function update_player_stats(&$players,$account_id,&$ships)
{
	...
	if($num_kills)
	{
		...
		$currPlayer->increaseHOF($players[$account_id][EXPERIENCE_GAINED] + $players[$account_id][EXPERIENCE_LOST] - ceil($players[$account_id][TOTAL_DAMAGE] * 0.25),array('Killing','Experience','Gained'));
		$currPlayer->increaseHOF($traders_killed_exp,array('Killing','Experience','Of Traders Killed'));
		$currPlayer->increaseHOF($players[$account_id][CREDITS_GAINED],array('Killing','Money','Gain'));
		$currPlayer->increaseHOF($killed_ships,array('Killing','Money','Cost Of Ships Killed'));
	}
	$currPlayer->update();
}
...
if(count($killed_ids)) {
	...
	if($player->getAllianceID() && $players[$var['target']][ALLIANCE_ID]) {
		...
		if($db->next_record()) {
			$db->query('UPDATE alliance_vs_alliance SET kills=kills+' . $fleets[0][0] . ' WHERE alliance_id_1=' . $player->getAllianceID() . ' AND alliance_id_2=' . $players[$var['target']][ALLIANCE_ID] . ' AND game_id=' . SmrSession::$game_id . ' LIMIT 1');
		}
		else {
		...
		if($db->next_record()) {
			...
			$db->query('INSERT INTO alliance_vs_alliance VALUES (' . SmrSession::$game_id . ',' . $players[$var['target']][ALLIANCE_ID] . ',' . $player->getAllianceID() . ',' . $fleets[1][0] . ')');
		}
	}
}
