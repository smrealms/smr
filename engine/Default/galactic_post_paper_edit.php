...
$db->query('SELECT * FROM galactic_post_paper WHERE paper_id = '.$var['id'].' AND game_id = ' . $db->escapeNumber($player->getGameID()));
$db->nextRecord();
$paper_title = bbifyMessage($db->getField('title'));
$db->query('SELECT * FROM galactic_post_paper_content WHERE paper_id = '.$var['id'].' AND game_id = ' . $db->escapeNumber($player->getGameID()));
require_once(get_file_loc('menu.inc'));
create_galactic_post_menu();
$PHP_OUTPUT.=($paper_title.'<br /><br /><ul>');
while ($db->nextRecord()) {
	$db2->query('SELECT * FROM galactic_post_article WHERE game_id = ' . $db->escapeNumber($player->getGameID()) . ' AND article_id = ' . $db->getField('article_id'));
	$db2->nextRecord();
