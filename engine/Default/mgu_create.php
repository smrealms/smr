...
$alliance = '(0';
$db->query('SELECT * FROM player WHERE alliance_id = ' . $db->escapeNumber($player->getAllianceID()) . ' AND alliance_id != 0 LIMIT 1');
while ($db->nextRecord()) {
	$alliance .= ',';
...
while ($db->nextRecord()) {
	...
	if ($db2->getNumRows() > 0) {
		...
		$db2->query('SELECT * FROM player_visited_sector WHERE sector_id = '.$left.' AND game_id = '.$game_id.' AND account_id = ' . $db->escapeNumber($player->getAccountID()) . ' LIMIT 1');
