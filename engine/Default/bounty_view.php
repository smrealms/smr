...
if($curr_player->hasBounties())
{
	...
	foreach($bounties as $bounty)
	{
		 if($bounty['Type'] == 'HQ')
		 {
		 	$PHP_OUTPUT.=('The <font color=green>Federal Government</font> is offering a bounty on '.$curr_player->getPlayerName().' worth <font color=yellow>'.$bounty['Amount'].'</font> credits.<br />');
		 	if ($bounty['Claimer'] != 0)
		 	{
	...
	foreach($bounties as $bounty)
	{
		 if($bounty['Type'] == 'UG')
		 {
			$PHP_OUTPUT.=('The <font color=red>Underground</font> is offering a bounty on '.$curr_player->getPlayerName().' worth <font color=yellow>'.$bounty['Amount'].'</font> credits.<br />');
		 	if ($bounty['Claimer'] != 0)
		 	{
			 	$claiming_player =& SmrPlayer::getPlayer($bounty['Claimer'], $player->getGameID());
			 	$PHP_OUTPUT.=('This bounty can be claimed by '.$claiming_player->getPlayerName().'<br />');
