...
if ($player->isGPEditor()) {
	...
	if ($db->getNumRows()) {
		$PHP_OUTPUT.=('The following papers are already made (papers must have 3-8 articles to go to the press):<br /><br />');
	}
	while($db->nextRecord()) {
		...
		$hasEnoughArticles = $db2->getNumRows() > 2 && $db2->getNumRows() < 9;

		// Use red/green to show if a paper has enough articles
		$PHP_OUTPUT.=(' which contains <span class="' . ($hasEnoughArticles ? "green" : "red") . '"> ' . $db2->getNumRows() . ' </span>articles. ');
		if (!empty($published)) {
			$PHP_OUTPUT.=('<span class="bold green">PUBLISHED!</span>');
