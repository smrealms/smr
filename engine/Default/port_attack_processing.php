<?
echo 'TELL PAGE YOU SAW THIS (PORT_ATTACK_PROCESSING)';
...
//}
//
//if ($att_time == 0) $db->query('INSERT INTO port_attack_times (game_id, sector_id, time) VALUES ('.$player->getGameID().', '.$sector->getSectorID().', '.$time.')');
//else $db->query('UPDATE port_attack_times SET time = '.$time.' WHERE game_id = '.$player->getGameID().' AND sector_id = '.$player->getSectorID());
//$db->unlock();
//
//// take the turns
...
//
//	$port->shields = ($port->getLevel() * 1000 + 1000) + ($rich_mod * 500);
//	$port->armor = ($port->getLevel() * 1000 + 1000) + ($rich_mod * 500);
//	$port->drones = ($port->getLevel() * 100 + 100) + ($rich_mod * 50);
//
//	$port->attack_started = $time;
//	$port->getReinforceTime() = $time + ($port->getLevel() * 5 * 60);
//	$db->query('DELETE FROM player_attacks_port WHERE game_id = '.SmrSession::$game_id.' AND sector_id = '.$sector->getSectorID());
//
//	//insert into the news that the port is being attacked.
//	if ($player->getAllianceID() != 0)
//		$attack_news = 'members of $player->getAllianceName()';
//	else
//		$attack_news = $player->getDisplayName();
//	$news_message = '<span style="color:red;">*MAYDAY* *MAYDAY*</span> The port in sector <span style="color:yellow;">'.$sector->getSectorID().'</span> is being attacked by '.$attack_news.'. Immediate backup requested!';
//	$db2->query('INSERT INTO news ' .
//				'(game_id, time, news_message) ' .
//				'VALUES('.SmrSession::$game_id.', ' . time() . ', ' . $db->escape_string($news_message, false) . ')');
...
//}
//
//$attacker_team = new SMR_BATTLE($player->getAccountID(), $player->getGameID(), TRUE);
//
//// ********************************
//// *
//// * P o r t	s h o o t s
...
//	$curr_attacker_ship = new SMR_SHIP($curr_attacker->getAccountID(), SmrSession::$game_id);
//	if ($curr_attacker->dead == 'TRUE') {
//
//		$port_msg[] = 'The port fires at the debris that used to be <span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span>.';
//		continue;
//
//	}
//	$port_acc = ($port->getLevel() * 10) - ($curr_attacker->level_id / 2);
//	if ($port_acc < 0)
//		$port_acc = 0;
//
//	// would the port hit?
...
//			$curr_attacker_ship->getShields() -= $damage;
//
//			// message
//			$port_msg[] = 'The port fires a turret at <span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> and destroys <span style="color:red;">'.$damage.'</span> shields';
//
...
//			if ($damage > $curr_attacker_ship->getCDs())
//				$damage = $curr_attacker_ship->getCDs();
//
//			// accumulate the attacker_damage
//			$port_damage += $damage;
//
//			// subtract the armor damage
//			$curr_attacker_ship->getCDs() -= $damage;
//
//			// message
//			$port_msg[] = 'The port fires a turret at <span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> and destroys <span style="color:red;">'.$damage.'</span> drones';
//
...
//
//			// subtract the armor damage
//			$curr_attacker_ship->getArmour() -= $damage;
//
//			// message
//			$port_msg[] =   'The port fires a turret at <span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> and destroys <span style="color:red;">'.$damage.'</span> armor';
//
//		}
//
//		// is he dead now?
//		if ($curr_attacker_ship->getShields() == 0 && $curr_attacker_ship->getArmour() == 0 && !in_array($curr_attacker->getAccountID(), $dead_traders)) {
//
//			// write a message to the array
//			$port_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> has been <span style="color:red;">DESTROYED</span> by port forces';
//
//			if ($curr_attacker->getAccountID() != $player->getAccountID())
//				$curr_attacker->set_info('dead', 'TRUE');
//
//			// add him to the array to process later
//			$dead_traders[] = $curr_attacker->getAccountID();
//
//		}
//
//		//update the hardware
...
//
//	} else
//		$port_msg[] = 'The port fires a turret at <span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> and misses';
//
//}
//
//if ($port->drones > 0) {
...
//
//		// do we do more damage than shields left?
//		if ($drones_to_shield > $curr_attacker_ship->getShields())
//			$drones_to_shield = $curr_attacker_ship->getShields();
//
//		// accumulate the attacker_damage
//		$port_damage += $drones_to_shield;
//
//		// subtract the shield damage
//		$curr_attacker_ship->getShields() -= $drones_to_shield;
//
...
//	if ($curr_attacker_ship->getCDs() > 0 && $damage > 0) {
//
//		// assume we use all the damage to kill his drones
//		$drones_to_drones = $damage;
//
//		// do we do more damage than drones left?
//		if ($drones_to_drones > $curr_attacker_ship->getCDs() * 3)
//			$drones_to_drones = $curr_attacker_ship->getCDs() * 3;
//
//		// accumulate the attacker_damage
//		$port_damage += $drones_to_drones;
//
//		// subtract the drone damage
//		$curr_attacker_ship->getCDs() -= round($drones_to_drones / 3);
//
...
//
//	// does the attacker has armor left?
//	if ($curr_attacker_ship->getArmour() > 0 && $damage > 0) {
//
//		// assume we use all the damage to kill his armor
//		$drones_to_armor = $damage;
//
//		// do we do more damage than armor left?
//		if ($drones_to_armor > $curr_attacker_ship->getArmour())
//			$drones_to_armor = $curr_attacker_ship->getArmour();
//
//		// accumulate the attacker_damage
//		$port_damage += $drones_to_armor;
//
//		// subtract the armor damage
//		$curr_attacker_ship->getArmour() -= $drones_to_armor;
//
...
//	}
//
//	$msg = 'The port launches $num_att drones at <span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> ';
//
...
//	$curr_attacker->relations[$port->getRaceID()] -= 5;
//	if ($curr_attacker->relations[$port->getRaceID()] < -500)
//		$curr_attacker->relations[$port->getRaceID()] = -500;
//
...
//
//					// text
//					$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' destroying <span style="color:red;">'.$shield_damage.'</span> port shields.';
//
...
//
//					if ($drones_hit > 0)
//						$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' destroying <span style="color:red;">'.$drones_hit.'</span> port drones.';
//
//				} else
//					$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' which does no damage against the port shields.';
//
...
//
//					// echo message
//					$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' destroying <span style="color:red;"> ' . round ($armor_damage / 3) . ' </span> port drones.';
//
//				} elseif ($shield_damage > 0)
//					$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' which proves to be ineffective against the port drones.';
//
...
//
//					// text
//					$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' destroying <span style="color:red;">'.$armor_damage.'</span> plates of armor.';
//
//				} elseif ($shield_damage > 0)
//					$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' which proves to be ineffective against the ports armor.';
//			}
//
//		} else
//			$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> fires a '.$weapon_name.' and misses.';
//
//	} // end of weapons
//
//	// do we have drones?
//	if ($curr_attacker_ship->getCDs() > 0 && ($port->shields > 0 || $port->drones > 0 || $port->armor > 0)) {
//
...
//		$number_attacking = round($percent_attacking * $curr_attacker_ship->getCargoHolds());
//
...
//
//		// build text
//		$msg = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> launches ' . ceil($killed_shields / 2 + $killed_drones * 3 / 2 + $killed_armor / 2) . ' drones hitting the port and destroying ';
//
//		if ($killed_shields > 0) {
//
//			if ($killed_shields == 1)
...
//			$damage_msg[] = '<span style="color:yellow;">'.$curr_attacker->getPlayerName().'</span> claims <span style="color:yellow;">'.$port->credits.'</span> credits.';
//			$port->credits = 0;
//
//			// news message
//			if ($player->getAllianceID() != 0)
...
//			else
//				$attack_news = $player->getDisplayName();
//			$news_message = '.$db->escapeString('.$attack_news.' successfully raided the port located in sector '.$player->getSectorID();
...
//			'('.$player->getAccountID().', '.$player->getGameID().', NULL, \'HQ\', '.$amount.', 0, '.$time.')');
//	}
//	$player->get_relations();
//	if($port->getRaceID() > 1) {
//		$new_relations = $player->relations[$port->getRaceID()] - 45;
//		if ($new_relations < -500) $new_relations = -500;
//
//		$db->query('REPLACE INTO player_has_relation (account_id, game_id, race_id, relation) VALUES('.$player->getAccountID().', '.$player->getGameID().', '.$port->getRaceID().', '.$new_relations.')');
//	}
//
//	// also we change alignment
//	if ($player->relations_global_rev[$port->getRaceID()] < -299)
//	   $new_alignment = $player->getAlignment() + $port_original_level * 2;
//	else
//	   $new_alignment = $player->getAlignment() - $port_original_level * 2;
