...
$query = 'SELECT
	...
	min(sender_player_id) as sender_player_id,
...
if ($db->getNumRows() > 0) {
	...
	while ($db->nextRecord()) {
		...
		$senderPlayerID = $db->getInt('sender_player_id');
		if ($senderPlayer == PLAYER_ID_PLANET) {
			$playerName = 'Planet Reporter';
		} elseif ($senderPlayerID == PLAYER_ID_BANK_REPORTER) {
			$playerName = 'Bank Reporter';
		} elseif ($senderPlayerID == PLAYER_ID_ADMIN) {
			$playerName = 'Game Admins';
		} else {
			try {
				$author = SmrPlayer::getPlayer($senderPlayerID, $player->getGameID());
		...
		$db2->nextRecord();
		$threads[$i]['CanDelete'] = $player->getPlayerID() == $senderPlayerID || $db2->getBoolean('mb_messages');
