...
{
	if (DEBUG) print("Entered planet array<br>");
	global $db, $session, $player, $account;
	$db->query("SELECT  * FROM planet WHERE sector_id = " . $player->sector_id . " AND game_id = " . $player->game_id);
	if ($db->next_record()) {
		...
		if (DEBUG) print("Level set to " . $planet[PLANET_LEVEL] . "<br>");
...
{
	if (DEBUG) print("Entered player array<br>");
	global $db, $session, $player, $account;
	//insert our trigger into the players array
	$query = "SELECT * FROM player WHERE account_id = " . $player->account_id . " AND game_id = " . $player->game_id;
	$db->query($query);
	$db->next_record();
...
//	}
	...
	while ($db->next_record()) {
		if ($db->f("alliance_id_1") == $player->alliance_id) $helperAlliances[] = $db->f("alliance_id_2");
...
{
	if (DEBUG) print("Get Hardware<br>");
	global $db, $session, $player, $account;
...
function getFleet(&$players,&$weapons) {
	...
	foreach ($player_ids as $account_id) {
		...
		else if($account_id != $player->account_id) {
...
function protected_rating($account_id,&$players,&$weapons) {
	if (DEBUG) print("Checking Protected Rating<br>");
...
{
	if (DEBUG) print("Planet Fires<br>");
	global $db,$session,$player;
	//Turrets Fire
...
function planetFiresTurret($attackers, $planet, &$players, $target) {
	if (DEBUG) print("Planet Fires its turret<br>");
...
{
	if (DEBUG) print("Planet Fires drones<br>");
	$result = array(0,0,0,0,NORMAL_HIT);
...
{
	if (DEBUG) print("Player fires<br>");
	global $db,$session,$player;
...
{
	if (DEBUG) print("Player Fires Weap<br>");
...
function processNews($fleet, $planet) {
	if (DEBUG) print("News Process<br>");
	global $db, $player;
	$allowed = TIME - 600;
	$db->f("SELECT * FROM news WHERE type = 'breaking' AND game_id = $player->game_id AND time > $allowed");
	if (!$db->nf()) {
		if (sizeof($fleet) >= 5) {
			$db->query("SELECT  * FROM player WHERE game_id = $player->game_id AND account_id = " . $planet[OWNER]);
			$db->next_record();
			$text = sizeof($fleet) . " members of $player->alliance_name have been spotted attacking " .
					$planet[PLANET_NAME] . " in sector " . $player->sector_id . "
					.  The planet is owned by " . stripslashes($db->f("player_name"));
			if ($db->f("alliance_id") > 0) {
				$db->query("SELECT * FROM alliance WHERE alliance_id = " . $db->f("alliance_id") . " AND game_id = $player->game_id");
				$db->next_record();
...
function hofTracker($players, $planet) {
	...
	foreach ($players as $accId => $crap) {
		$db->query("REPLACE INTO player_attacks_planet (game_id, account_id, sector_id, time, level) VALUES " .
					"($player->game_id, $accId, $player->sector_id, " . TIME . ", " . round($planet[PLANET_LEVEL]) . ")");
	}
}
function processResults($players, $planet, $fleet, $weapons) {
	if (DEBUG) print("Processing Results<br>");
	global $db, $player;
			...
			if ($resultArray[RESULT_OF_WEAPON] == PLANET_DEAD) {
				...
				foreach ($temp as $tempAcc => $level) {
					$db->query("UPDATE player_has_stats SET planet_busts = planet_busts + 1, planet_bust_levels = planet_bust_levels + $level " . 
								"WHERE account_id = $tempAcc AND game_id = $player->game_id");
					$db->query("UPDATE account_has_stats SET planet_busts = planet_busts + 1, planet_bust_levels = planet_bust_levels + $level " . 
								"WHERE account_id = $tempAcc");
				}
...
function podPlayers($IDArray, $ships, $hqs, $planet, $players) {
	if (DEBUG) print("Podding Players<br>");
	global $db, $session, $player;
	...
	foreach ($IDArray as $accId) {
		...
		$db->query("UPDATE player SET ship_type_id = 69, turns = $turns, newbie_turns = 100, " . 
					...
					"WHERE game_id = $player->game_id AND account_id = $accId");
...
function sendReport($results, $planet) {
	if (DEBUG) print("Sending Reports<br>");
	global $player, $db;
	$db->query("SELECT * FROM player WHERE account_id = " . $planet[OWNER] . " AND game_id = $player->game_id LIMIT 1");
	$db->next_record();
	...
	if ($ownerAlliance > 0) {
		...
		if ($thread_id == 0)
		{
			$db->query("SELECT * FROM alliance_thread_topic WHERE game_id = $player->game_id AND alliance_id = $ownerAlliance ORDER BY thread_id DESC LIMIT 1");
			if ($db->next_record())
				$thread_id = $db->f("thread_id") + 1;
			else $thread_id = 1;
			$db->query("INSERT INTO alliance_thread_topic (game_id, alliance_id, thread_id, topic) VALUES " .
						"($player->game_id, $ownerAlliance, $thread_id, '$topic')");
		}
		$db->query("SELECT * FROM alliance_thread WHERE alliance_id = $ownerAlliance AND game_id = $player->game_id AND " .
		...
		if ($thread_id == 0)
		{
			$db->query("SELECT * FROM alliance_thread_topic WHERE game_id = $player->game_id AND alliance_id = $player->alliance_id ORDER BY thread_id DESC LIMIT 1");
			if ($db->next_record())
				$thread_id = $db->f("thread_id") + 1;
			else $thread_id = 1;
			$db->query("INSERT INTO alliance_thread_topic (game_id, alliance_id, thread_id, topic) VALUES " .
						"($player->game_id, $player->alliance_id, $thread_id, '$topic')");
		}
		$db->query("SELECT * FROM alliance_thread WHERE alliance_id = $player->alliance_id AND game_id = $player->game_id AND " .
					"thread_id = $thread_id ORDER BY reply_id DESC LIMIT 1");
		if ($db->next_record()) $reply_id = $db->f("reply_id") + 1;
		else $reply_id = 1;
		$db->query("INSERT INTO alliance_thread (game_id, alliance_id, thread_id, reply_id, text, sender_id, time) VALUES " .
...
function doLog($results) {
	if (DEBUG) print("Logging<br>");
	global $account, $player;
	$account->log(12, "Player attacks planet their team does " . $results[TOTAL_PLAYER_DMG], $player->sector_id);
}
function checkContinue($players, $planet) {
	if (DEBUG) print("Continue?<br>");
	global $player;
	if ($players[$player->account_id][KILLER]) return FALSE;
	if ($planet[PLANET_SHIELDS] == 0 && $planet[PLANET_DRONES] == 0) return FALSE;
	return TRUE;
}
if ($player->newbie_turns) create_error("You can't shoot at that planet while under newbie protection.");
if ($player->turns < 3) create_error("Insufficient turns to perform that action");
if (DEBUG) print("Opening<br>");
$planet = getPlanetArray();
if (!$planet[OWNER]) require(get_file_loc('planet_defender_check.php'));
if (DEBUG) print("Level is still set to " . $planet[PLANET_LEVEL] . "<br>");
...
if (!sizeof($players[$player->account_id][WEAPONS])) create_error("What are you going to do?  Insult it to death?");
//in case there are more than 10 players IS.  $fleet contains account_ids of the attackers
$fleet = getFleet($players, $weapons);
if (DEBUG) print("Pre news<br>");
...
if ($players[$player->account_id][KILLER]) $container['override_death'] = TRUE;
