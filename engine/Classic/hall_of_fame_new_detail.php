...
while ($db->next_record()) {
	...
	if ($db->f("account_id") == $session->account_id) $bold = " style=\"font-weight:bold;\"";
	else $bold = "";
	print("<tr>");
...
if ($cat == "<b>Money Donated to SMR</b>")
	$db->query("SELECT account_id, sum(amount) as amount FROM account_donated " .
			"WHERE account_id = $session->account_id GROUP BY account_id");
else
	$db->query("SELECT account_id, $row as amount FROM $table " .
			"$row > 0 AND account_id = $session->account_id ORDER BY amount DESC");
if ($db->next_record()) {
