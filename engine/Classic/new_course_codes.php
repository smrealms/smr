...
while ($db->next_record()) {
	...
	$sector_links = array();
	$sector_links[1] = $db->f("link_up");
	$sector_links[2] = $db->f("link_right");
	$sector_links[3] = $db->f("link_down");
	$sector_links[4] = $db->f("link_left");
	$db2->query("SELECT * FROM warp WHERE game_id = $game_id AND (sector_id_1 = $sec_id OR sector_id_2 = $sec_id)");
	if ($db->next_record()) {
			
		if ($db2->f("sector_id_1") == $sec_id)
			$sector_links[5] = $db2->f("sector_id_2");
		else
			$sector_links[5] = $db2->f("sector_id_1");
	} else
		$sector_links[5] = 0;
...
$explored = array();
$db->query("SELECT * FROM sector WHERE game_id = $player->game_id ORDER BY sector_id");
while ($db->next_record()) {
	...
	$db3->query("SELECT * FROM alliance WHERE game_id = $player->game_id AND " .
...
$db->query("REPLACE INTO alliance_maps (game_id, alliance_id, maps) VALUES " .
				"($player->game_id, $player->alliance_id, $explored)");
...
$db->query("DELETE FROM player_has_maps WHERE game_id = $player->game_id AND account_id = $player->account_id");
...
$db->query("SELECT * FROM alliance_has_maps WHERE game_id = $player->game_id AND " .
				"alliance_id = $player->alliance_id");
$old_maps = $db->f("maps");
$db->query("REPLACE INTO player_has_maps (game_id, account_id, maps) VALUES " .
				"($player->game_id, $player->account_id, $old_maps)");
...
$db->query("SELECT * FROM sector WHERE sector_id = $to AND game_id = $session->game_id");
if ($db->nf() == 0)
	create_error("That sector doesn't exist. Try a new one.");

$db->query("SELECT * FROM sector WHERE sector_id = $from AND game_id = $session->game_id");
if ($db->nf() == 0)
	create_error("That sector doesn't exist. Try a new one.");
	
//get our arrays
$db->query("SELECT * FROM universe_array WHERE game_id = $player->game_id");
$uni_array = $db->f("array");
if ($player->alliance_id != 0)
	$db->query("SELECT * FROM alliance_maps WHERE game_id = $player->game_id AND alliance_id = $player->alliance_id");
else
	$db->query("SELECT * FROM player_has_maps WHERE game_id = $player->game_id AND " .
					"account_id = $player->account_id");
					
$user_array = $db->f("maps");
...
while (sizeof($sector_queue) > 0) {
	...
	if (in_array($curr_sec_id, $user_array) && $curr_sec_id != $to) {
		...
		unset($sector_distance[$curr_sector->sector_id]);
		continue;
	}
	
	// get the distance for this sector from the source
	$distance = $sector_distance[$curr_sector->sector_id];
