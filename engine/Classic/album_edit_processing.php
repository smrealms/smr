...
if ($db->next_record()) {
	...
	$db->query("INSERT INTO album (account_id, location, email, website, day, month, year, other, created, last_changed, approved) " .
			   "VALUES($session->account_id, $location, $email, '$website', $day, $month, $year, $other, $curr_time, $curr_time, 'TBC')");
...
if ($comment) {
	...
	$db->query("SELECT MAX(comment_id) FROM album_has_comments WHERE album_id = $session->account_id");
	if ($db->next_record())
