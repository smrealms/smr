...
{
	if (DEBUG) print("Entered port array<br>");
	global $db, $player;
...
function portFiresTurret($attackers, $port, &$players, $target) {
	if (DEBUG) print("Port Fires its turret<br>");
...
{
	if (DEBUG) print("Port Fires drones<br>");
	$result = array(0,0,0,0,NORMAL_HIT);
...
function playerFiresWeapon($weapon,$attacker,&$port,&$players,&$weapons)
{
	...
	if($port[PORT_SHIELDS] == 0 && $port[PORT_ARMOR] == 0) {
	...
	if($port[PORT_SHIELDS] != 0 ) {
		...
		if($potential_damage) {
			...
			if ($weapon && $weapons[$weapon][ARMOR_DAMAGE] && $port[PORT_DRONES]) {
				if ($weapons[$weapon][ARMOR_DAMAGE] > $port[PORT_DRONES] * 60)
	...
	if($port[PORT_ARMOR] != 0 ) {
		...
		if($potential_damage) {
			...
			if($potential_damage >= $port[PORT_ARMOR]) {
				...
				$result[ARMOR_DMG_DONE] = $potential_damage;
				$result[RESULT_OF_WEAPON] = NORMAL_HIT;
				return $result;
			}
		} else {
			$result[RESULT_OF_WEAPON] = SHIELD_ON_ARMOR;
			return $result;
		}
	}
	return $result;
...
function processNews() {
	...
	$text .= " <span class=\"yellow\">$player->sector_id</span>.";
	if ($player->alliance_id) {
		$text .= "  It is under attack by members of <span class=\"yellow\">$player->alliance_name</span>. ";
		$text .= " The Federal Government is offering various bounties for the deaths of any raiding members of <span class=\"yellow\">$player->alliance_name</span>";
	} else {
		$text .= "  It is under attack by <span class=\"yellow\">$player->player_name</span>";
...
function hofTracker($players, $port) {
	...
	foreach ($players as $accId => $crap) {
		$db->query("REPLACE INTO player_attacks_port (game_id, account_id, sector_id, time, level) VALUES " .
					"($player->game_id, $accId, $player->sector_id, " . TIME . ", " . $port[PORT_LEVEL] . ")");
	}
}
function processResults(&$players, &$port, $fleet, $weapons) {
	...
	foreach ($fleet as $accId) {
			...
			if ($resultArray[RESULT_OF_WEAPON] == FINAL_HIT) {
				...
				foreach ($temp as $tempAcc => $level) {
					$db->query("UPDATE player_has_stats SET port_raids = port_raids + 1, port_raid_levels = port_raid_levels + $level " . 
								"WHERE account_id = $tempAcc AND game_id = $player->game_id LIMIT 1");
				...
				if ($db->next_record() && $amount > 0) {
					...
					$db->query("INSERT INTO bounty (account_id, game_id, type, amount, claimer_id, time) VALUES " .
						"($player->account_id, $player->game_id, 'HQ', $amount, 0, " . TIME . ")");
				}
		...
		$traderDisplay = "<span style=\"color:yellow;font-variant:small-caps\">Port " . $player->sector_id . "</span>'s defenses " . 
...
function portDowngrade(&$results, &$port) {
	...
	if ($port[PORT_LEVEL] > 1) {
		...
		for ($i = 0; $i < $numberOfChances; $i++) {
			if (mt_rand(1, 100) <= 5) {
				$db->query("SELECT count(*) as numGoods FROM port_has_goods WHERE game_id = $player->game_id AND sector_id = $player->sector_id");
				$db->next_record();
				if ($db->f("numGoods") >= 3) {
...
function doLog($results) {
	...
	$account->log(7, "Player attacks port their team does " . $results[TOTAL_PLAYER_DMG], $player->sector_id);
}
...
if ($player->newbie_turns) create_error("You can't shoot at that port while under newbie protection.");
...
$db->query('INSERT INTO combat_logs VALUES("",' . SmrSession::$game_id . ',"PORT",' . $player->sector_id . ',' . time() . ',' . SmrSession::$account_id . ',' . $player->alliance_id . ',0,0,"' . mysql_real_escape_string(gzcompress($finalResults)) . '", "FALSE")');
