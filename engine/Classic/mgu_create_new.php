...
/*
 * Script:
 * 		/engine/Classic/mgu_create_new.php
 * 
 * Changelog:
...
$db->query(
	...
	'FROM sector WHERE game_id=' . $player->game_id . ' ' .
...
$db->query(
	...
	'WHERE location.game_id=' . $player->game_id . ' ' .
...
$db->query(
	...
	'WHERE port.game_id=' . $player->game_id . ' ' .
	'AND player_visited_port.game_id=' . $player->game_id . ' ' .
	'AND port.sector_id = player_visited_port.sector_id ' .
	'AND player_visited_port.account_id=' . $player->account_id
);
...
if($player->alliance_id) {
	$query .= '(player.alliance_id = ' . $player->alliance_id . ' OR ' .
	'sector_has_forces.owner_id = ' . $player->account_id . ' ) ';
}
else {
	$query .= 'sector_has_forces.owner_id = ' . $player->account_id . ' ';
}
...
if($player->alliance_id) {
	...
	$query .= 'planet.owner_id = ' . $player->account_id . ' ';
}		
