...
$db->query("SELECT message_id FROM bar_tender WHERE game_id = $session->game_id ORDER BY message_id DESC");
if ($db->next_record())
...
if (isset($gossip_tell))
	$db->query("INSERT INTO bar_tender (game_id, message_id, message) VALUES ($session->game_id, $amount,  " . format_string($gossip_tell, true) . " )");

$db->query("SELECT * FROM bar_tender WHERE game_id = $session->game_id ORDER BY rand() LIMIT 1");
