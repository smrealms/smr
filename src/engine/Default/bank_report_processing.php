...
if ($db->nextRecord()) {
	...
	$db->query('UPDATE alliance_thread SET time = ' . $db->escapeNumber(SmrSession::getTime()) . ', text = ' . $db->escapeString($text) . ' WHERE thread_id = ' . $db->escapeNumber($thread_id) . ' AND alliance_id = ' . $db->escapeNumber($alliance_id) . ' AND game_id = ' . $db->escapeNumber($player->getGameID()) . ' AND reply_id = 1');
