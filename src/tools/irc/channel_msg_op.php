 ...
 * @param resource $fp
 */
function channel_msg_op_info($fp, Message $msg, Player $player): bool {
	if ($msg->text === '!op info') {
...
function channel_msg_op_cancel($fp, Message $msg, Player $player): bool {
	...
	if ($msg->text === '!op cancel') {
		...
		$dbResult = $db->select('alliance_has_op', $alliance->SQLID);
 ...
 * @param resource $fp
 */
function channel_msg_op_turns($fp, Message $msg, Player $player): bool {
 ...
 * @param resource $fp
 */
function channel_msg_op_list($fp, Message $msg, Player $player): bool {
	if ($msg->text === '!op list') {
...
function channel_op_notification($fp, string $rdata, string $nick, string $channel): bool {
	...
	$dbResult = $db->select('alliance_has_op_response', [
		...$player->SQLID,
		'alliance_id' => $player->getAllianceID(),
	]);
	if (!$dbResult->hasRecord()) {
