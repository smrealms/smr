...
function getHofRank($view, $viewType, $accountID, $gameID) {
	...
	$gameIDSql = ' AND game_id ' . (isset($gameID) ? '= ' . $db->escapeNumber($gameID) : 'IN (SELECT game_id FROM game WHERE end_time < ' . SmrSession::getTime() . ' AND ignore_stats = ' . $db->escapeBoolean(false) . ')');
