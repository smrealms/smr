...
if (!empty($account_id))
{
	...
	{
		$account_id=array();
		$db->query('SELECT account_id FROM player WHERE game_id='.$game_id);
		while ($db->nextRecord())
		{
	...
	$db->query('DELETE FROM player_visited_sector WHERE account_id IN ('.$db->escapeArray($account_id).') AND game_id = '.$game_id);
